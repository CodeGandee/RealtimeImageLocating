...
int indirectMatchAndShow(IMGRGLOC_Image2MapIndirect* image2map_pipe, Mat cv_map, VideoCapture* cap)
{
    ...
    CVCMat cvc_image = &cv_image;
    CVCMat cvc_transform = &transform;
    ...
    while (1)
    {
        ...
        IMGRGLOC_Image2MapIndirect_get_ref_image(image2map_pipe,cvc_image);
        
        Mat cv_map_show = cv_map.clone();
        IMGRGLOC_Image2MapIndirect_get_transform_matrix(image2map_pipe, cvc_transform);
        //cout << transform << endl;
        if (match_success && !transform.empty())
        {
...
int main() {
    ...
    IMGRGLOC_Image2MapIndirect_set_map_maxlen(image2map_pipe, -1);
    IMGRGLOC_Image2MapIndirect_set_ref_image_maxlen(image2map_pipe, -1);
