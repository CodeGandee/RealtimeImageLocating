...
int64_t CVCMatGetByteCount(CVCMat mat){
   auto& _mat = *(cv::Mat*)mat;
   return _mat.total()*_mat.elemSize();
}

CVC_EXPORT CVCMat CVCMatCreateWithSize(int nrow, int ncol, int cvtype, char* data)
{
    cv::Mat* mat = nullptr;
    if(data)
        mat = new cv::Mat(nrow, ncol, cvtype, data);
    else
        mat = new cv::Mat(nrow, ncol, cvtype);
    return (CVCMat)mat;
}

void CVCMatCreateData(CVCMat mat, int nrow, int ncol, int cvtype)
{
    auto& _mat = *(cv::Mat*)mat;
    _mat.create(nrow, ncol, cvtype);
}

void CVCMatCopyFrom(CVCMat mat, char* data)
{
    auto& _mat = *(cv::Mat*)mat;
    cv::Mat(_mat.rows, _mat.cols, _mat.type(), data).copyTo(_mat);
}
...
{
    auto& _mat = *(cv::Mat*)mat;
    return (char*)_mat.data;
}

CVC_EXPORT char* CVCMatAt(CVCMat mat, int nbyte_per_pixel, int i, int j)
{
    auto& _mat = *(cv::Mat*)mat;
    char* ptr = (char*)(_mat.data + (i * _mat.cols + j) * nbyte_per_pixel);
    return ptr;
}
