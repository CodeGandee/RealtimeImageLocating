...
namespace imgregionloc {
	...
	private:
		...
		SIFTMacherParam m_init_param;

	public:
		SIFTRegionMatcher() : m_map(new ImageWithFeature), m_query(new ImageWithFeature) {
			init_with_params(m_init_param);
		}
		void init_with_params(const SIFTMacherParam& p);
	...
	using SIFTRegionMatcherPtr = std::shared_ptr<SIFTRegionMatcher>;
}
