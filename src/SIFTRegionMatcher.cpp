...
namespace imgregionloc {
    void SIFTRegionMatcher::init_with_params(const SIFTMacherParam& p)
    {
        matcher = DescriptorMatcher::create(p.m_type);
        m_upper_limit = p.m_most_mpt;
        m_lower_limit = p.m_least_mpt;
    }

    void SIFTRegionMatcher::set_map_feature(ImageWithFeaturePtr map_features)
    {
        m_map = map_features;
    ...
    void SIFTRegionMatcher::set_query_image_feature(ImageWithFeaturePtr query_features)
    {
        m_query = query_features;
    }

    void SIFTRegionMatcher::process_match()
    {
    ...
    void SIFTRegionMatcher::get_matched_points_in_map(PointList2& out)
    {
        out = m_map_pt;
    }
    void SIFTRegionMatcher::get_matched_points_in_image(PointList2& out)
    {
        out = m_query_pt;
    }
    void SIFTRegionMatcher::get_perspective_transform_matrix(cv::Mat& perspective_transform_matrix) const
    {
        perspective_transform_matrix = m_perspective_transform_matrix.clone();
    }
    void SIFTRegionMatcher::check_match_success(bool& success) const
    {
    ...
    void SIFTRegionMatcher::get_confidence(float& confidence) const
    {
        float raw_c = 0.0f;
